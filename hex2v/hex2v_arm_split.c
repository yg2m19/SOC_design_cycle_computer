...
//
// Usage:
//    hex2v_arm_split <infile> <bytes_per_word> [ <num_banks> <this_bank> ]
//
...
int print_formated_hex_data (int index, int index_divisor ) {
  ...
  //     @<HEX_ADDR> <HEX_HATA>
  //   
  printf( "@%04X ", address/index_divisor);
...
int print_usage_and_exit() {
  ...
  fprintf( stderr, "Usage: hex2v_arm <infile> <bytes_per_word> [ <num_banks> <this_bank> ]\n");
  fprintf( stderr, "\n");
...
int main (int argc, char *argv[])
{
   ...
   if (argc == 5) {
	...
	NumBanks = 4;
       ...
       fprintf( stderr, "ERROR - expected banks == 1, 2 or 4.\n", argv[1]);
       exit( EXIT_FAILURE);
     }
	...
	ThisBank = 3;
       ...
       fprintf( stderr, "ERROR - expected bank # == 0, 1, 2 or 3.\n", argv[1]);
       exit( EXIT_FAILURE);
     }
   }
   ...
   for (i = ThisBank; i < num_words_stored; i = i + NumBanks) {
   
      print_formated_hex_data (i, NumBanks );
