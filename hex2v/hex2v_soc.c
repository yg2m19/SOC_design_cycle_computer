...
//
// Usage:
//    hex2v_soc <infile> <bytes_per_word> [<rom_name_for_assign>]
//
...
int print_usage_and_exit() {
  ...
  fprintf( stderr, "Usage: hex2v_soc <infile> <bytes_per_word> [<rom_name_for_assign>]\n");
  fprintf( stderr, "\n");
...
int main (int argc, char *argv[])
{
   ...
   while (!feof(IntelHexFile)) {
      ...
      if (strlen(IntelHexLine) >= 10) {
         // This is the ARM/PicoRV32, with its 16/32-bit instruction words and 32-bit data words.
